# -*- coding: utf-8 -*-
"""
ABLATION STUDY: CRISIS PERIOD (2008-2012)
==========================================
Extension 1: Different market regime

Dataset: Same 20 stocks, crisis period (2008-2012)
Period: Financial crisis (high volatility ~35%)
Configs: mu_only, sigma_only, mu_sigma, mu_random
Seeds: 3

Tests if findings generalize across market regimes:
  - Normal period (2010-2024): moderate volatility
  - Crisis period (2008-2012): extreme volatility

Expected: Same finding (mu_sigma ≈ mu_random)
Proves generalization across market conditions.
"""
...
# ============================================================================
# PHASE 2: CALIBRATION CORRECTION
# Based on Phase 1 findings: Kalman 95% CI covers only 60%
# Solution: Inflate sigma by 1.8x to achieve proper coverage
# ============================================================================

def apply_calibration_correction(beliefs_np, calibration_factor=1.8):
    """
    Apply calibration correction to Kalman beliefs.
    
    Phase 1 showed Kalman's uncertainty is under-estimated:
    - Target: 95% CI should cover 95% of returns
    - Actual: Only covers 60%
    - Fix: Inflate sigma by factor of 1.8
    
    Args:
        beliefs_np: numpy array of shape (T, 2*d)
                   First d columns: mu (mean predictions)
                   Last d columns: sigma (uncertainty estimates)
        calibration_factor: float, default 1.8 (from Phase 1 analysis)
    
    Returns:
        numpy array: Corrected beliefs with inflated sigma
    """
    import numpy as np
    
    # Split into mu and sigma
    T, total_dim = beliefs_np.shape
    d = total_dim // 2
    
    mu = beliefs_np[:, :d].copy()
    sigma = beliefs_np[:, d:].copy()
    
    # Inflate sigma (uncertainty)
    sigma_corrected = sigma * calibration_factor
    
    # Concatenate back
    beliefs_corrected = np.concatenate([mu, sigma_corrected], axis=1)
    
    return beliefs_corrected.astype(np.float32)
...
def train_one_run(
    ...
    run_dir = os.path.join(out_dir, "runs", f"belief{config_name}", f"lam{lam_tc:.6g}", f"seed{seed}")
    ensure_dir(run_dir)

    best_path = os.path.join(run_dir, f"best_seed{seed}_belief{config_name}_lam{lam_tc:.6g}.pth")
    ...
    for it in range(1, iters + 1):
        ...
        shrink_kappa = float(belief_shrink_kappa) if (config_name == "kalman") else 0.0
            ...
            if improved:
                ...
                meta = {
                    ...
                    "belief_stabilization": {
                        "belief_sigma_floor": float(belief_sigma_floor),
                        "belief_sigma_clip": float(belief_sigma_clip),
                        "belief_shrink_kappa": float(belief_shrink_kappa if config_name == "kalman" else 0.0),
                        "belief_z_clip": float(belief_z_clip),
                    },
                }
...
def main():
    ...
    # ========================================================================
    PRESET = 'conservative'  # PHASE 2: Fair comparison for all beliefs
    ...
    out_dir = "./ablation_crypto"  # CRYPTO  # ABLATION  # FAST TEST
    ...
    # beliefs
    config_names = ["mu_only", "sigma_only", "mu_sigma", "mu_random"]  # ABLATION
    print("[ABLATION] Testing 4 configurations:")
    print("  mu_only:    Mean predictions only")
    print("  sigma_only: Uncertainty only")
    print("  mu_sigma:   Both (current)")
    print("  mu_random:  Mu + random noise")
    print()  # PHASE 2: All three
    rolling_window = 60

    # seeds and lambda sweep
    # CRITICAL: Multiple seeds for statistical validity
    seeds = [0, 1, 2]  # FAST TEST  # Changed from [0] to 0-9
    lambda_sweep = [1e-3]  # FAST TEST
    lambda_main = 1e-3
    ...
    # -----------------------------
    # Baselines first (baselines don't use beliefs, but loop for consistency)
    # -----------------------------
    for seed in seeds:
        set_seed(seed)
        print(f"[BASELINES] seed={seed} lambda_tc_for_rebal={lambda_tc_for_rebal}")
        for config_name in config_names:  # ABLATION
            # For baselines, always use rolling (they don't actually use beliefs)
            ...
            for bname in baseline_names:
                ...
                print(f"[BASELINE] {bname} belief={config_name} seed={seed} "
    ...
    # -----------------------------
    # Policy runs: ABLATION x lambda sweep x seed
    # -----------------------------
    for config_name in config_names:  # ABLATION
        # Always use rolling beliefs as base
        beliefs_np_full = compute_rolling_beliefs(returns_np, window=rolling_window)
        valid_mask = np.zeros((T,), dtype=bool)
        valid_mask[rolling_window:] = True
        belief_shrink_kappa = 0.0
        
        # Split into mu and sigma
        mu = beliefs_np_full[:, :d]
        sigma = beliefs_np_full[:, d:]
        
        # Apply ABLATION configuration
        if config_name == "mu_only":
            beliefs_np = mu
            actual_branch_dim = d
            print(f"  [ABLATION] {config_name}: using mu only ({d} features)")
            
        elif config_name == "sigma_only":
            beliefs_np = sigma
            actual_branch_dim = d
            print(f"  [ABLATION] {config_name}: using sigma only ({d} features)")
            
        elif config_name == "mu_sigma":
            beliefs_np = beliefs_np_full
            actual_branch_dim = 2 * d
            print(f"  [ABLATION] {config_name}: using mu+sigma ({2*d} features)")
            
        elif config_name == "mu_random":
            # Mu + random noise (same magnitude as sigma)
            random_sigma = np.random.randn(T, d) * sigma.std()
            beliefs_np = np.concatenate([mu, random_sigma], axis=1)
            actual_branch_dim = 2 * d
            print(f"  [ABLATION] {config_name}: using mu+random ({2*d} features)")
        else:
        ...
        for lam_tc in lambda_sweep:
            for seed in seeds:
                print(f"[RUN] belief={config_name} lambda_tc={lam_tc} seed={seed} device={device}")

                model = DeepONetPolicy(
                    trunk_in_dim=2,
                    branch_in_dim=actual_branch_dim,  # ABLATION: d or 2*d
                    d_assets=d,
                    width=256,
                    depth=3,
                    act="silu",
    ...
    for config_name in config_names:  # ABLATION
        for lam_tc in lambda_sweep:
            pol = policy_test[(policy_test["belief"] == config_name) & (policy_test["lambda_tc"] == float(lam_tc))].copy()
            if pol.empty:
                continue

            for bname in baseline_names:
                base_method = f"baseline_{bname}"
                base = df_test[(df_test["method"] == base_method) & (df_test["belief"] == config_name)][
